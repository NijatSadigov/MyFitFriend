...
class UpdateDietaryLogsEntityUseCaseLB @Inject constructor(
    private val repository: MyFitFriendLocalRepository
){
operator fun invoke(dle:DietaryLogEntity):Flow<Resources<Int>> =flow{

    try {
        emit(Resources.Loading())

        // Modify the attribute before making the repository call
        dle.isSync = false

        val response = repository.editLog(dle)

        // Uncomment the below line if you want to see the response in the console
