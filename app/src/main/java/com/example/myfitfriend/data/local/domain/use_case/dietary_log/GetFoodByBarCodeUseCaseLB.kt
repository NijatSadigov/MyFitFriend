...
class GetFoodByBarCodeUseCaseLB @Inject constructor(
    private val repository: MyFitFriendLocalRepository
) {
    operator fun invoke(barCode: String): Flow<Resources<LocalFoodEntity>> = flow {

        try {
            emit(Resources.Loading())

            val response = repository.getFoodByQRCode(barCode)
