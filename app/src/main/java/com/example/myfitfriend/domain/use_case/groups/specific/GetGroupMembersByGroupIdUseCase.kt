...
class GetGroupMembersByGroupIdUseCase @Inject constructor(
    private val repository:MyFitFriendRepository
){
    operator fun invoke(groupId:Int):Flow<Resources<List<Int>>> =flow{

        try {
            emit(Resources.Loading())
            val response= repository.getGroupMembersByGroupId(groupId)
            println("response")
            println(response )
