...
class GetWorkoutByIdUseCase @Inject constructor(
    ...
    operator fun invoke(workoutId:Int):Flow<Resources<Workout>> =flow{

        try {
            emit(Resources.Loading<Workout>())
            ...
            emit(
                Resources.Error<Workout>(message =  e.localizedMessage?:"Unknown error occured")
            )
        }
        catch (e:IOException){
            emit(
                Resources.Error<Workout>(message = "could not determine err")
            )
        }
